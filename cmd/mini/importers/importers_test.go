...
func TestDetectProjectionKey(t *testing.T) {
	tests := []struct {
		...
		{"github url", ServerYAML{URL: "https://api.github.com/mcp"}, "github"},
		{"githubcopilot url", ServerYAML{URL: "https://api.githubcopilot.com/mcp"}, "github"},
		{"github cmd", ServerYAML{Command: "npx", Args: []string{"server-github"}}, "github"},
		{"slack url", ServerYAML{URL: "https://slack.com/mcp"}, "slack"},
		{"slack cmd", ServerYAML{Command: "npx", Args: []string{"server-slack"}}, "slack"},
		{"jira url", ServerYAML{URL: "https://myco.atlassian.net/mcp"}, "jira"},
		{"linear url", ServerYAML{URL: "https://linear.app/mcp"}, "linear"},
		{"sentry url", ServerYAML{URL: "https://mcp.sentry.io"}, "sentry"},
		{"unknown returns empty", ServerYAML{URL: "https://example.com"}, ""},
		{"empty returns empty", ServerYAML{}, ""},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := detectProjectionKey(tt.sc)
			if got != tt.want {
...
func TestInstallBundledProjection(t *testing.T) {
	t.Run("known server installs projection file", func(t *testing.T) {
		dir := tempDir(t)
		sc := ServerYAML{Name: "my-github", URL: "https://api.github.com/mcp"}

		InstallBundledProjection(dir, sc)
	...
	t.Run("unknown server installs nothing", func(t *testing.T) {
		dir := tempDir(t)
		sc := ServerYAML{Name: "unknown", URL: "https://example.com"}

		InstallBundledProjection(dir, sc)

		dest := filepath.Join(dir, "projections", "unknown.yaml")
		if _, err := os.Stat(dest); err == nil {
			t.Fatal("expected no projection file for unknown server")
		}
	})
	...
	t.Run("existing file is not overwritten", func(t *testing.T) {
		...
		sc := ServerYAML{Name: "my-slack", URL: "https://slack.com/mcp"}
		InstallBundledProjection(dir, sc)
