...
func newTestTokenEndpoint(t *testing.T) *testTokenEndpoint {
	...
	e.srv = httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		json.NewEncoder(w).Encode(map[string]any{"access_token": "refreshed", "refresh_token": "rotated", //nolint:errcheck
...
func assertToolCallOK(t *testing.T, resp map[string]any) {
	...
	if text, _ := item["text"].(string); !strings.Contains(text, "ok") {
		t.Errorf("tools/call text = %q, want ok", item["text"])
	}
}
...
func TestOAuthMidSession_toolCallSucceedsAfterTokenExpires(t *testing.T) {
	t.Run("rejected with 401", func(t *testing.T) {
	...
	t.Run("expired by clock", func(t *testing.T) {
		clk := clock.NewFake()
