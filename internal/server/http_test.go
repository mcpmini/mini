...
func assertAllowGetPost(t *testing.T, resp *http.Response) {
	t.Helper()
...
func TestHTTPServer_deleteReturns405(t *testing.T) {
	...
	assertAllowGetPost(t, resp)
}

func TestHTTPServer_putNotAllowed(t *testing.T) {
	_, ts := newHTTPTestServer(t)
	resp := requestMCPMethod(t, ts, http.MethodPut)
	defer resp.Body.Close()
	mustStatus(t, resp, http.StatusMethodNotAllowed)
}
...
func TestHTTPServer_DisallowedMethodsAllowHeader(t *testing.T) {
	_, ts := newHTTPTestServer(t)
