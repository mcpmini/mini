...
1. **Find it.** A `daemon.port` file records where a daemon *might* be. It's only a hint — liveness
   ...
   stored PID. (The `z` in `/healthz` is the Kubernetes/Google convention for an ops endpoint
   that's unlikely to collide with a real resource path.) `/healthz` reports
   `{"ok":true,"sessions":N}`.
...
- **Bearer token → browser rebinding + other local users.** `mini daemon` mints a 32-byte
  `crypto/rand` token and stores it `0600` (created with `O_EXCL`) in `daemon.token`. `/mcp`
